...
In this script, we implement 3 optimizers. We will optimize https://en.wikipedia.org/wiki/Rosenbrock_function which is a
benchmark problem in optimization.

Read this article:
https://towardsdatascience.com/a-visual-explanation-of-gradient-descent-methods-momentum-adagrad-rmsprop-adam-f898b102325c

Warning: Notations in the article are not the same as here. Try to forget about the article when doing this TP. Just
infer what has to be done from the __init__ andthe questions.

Then try to implement them here.

Some details are omitted, there is little chance that you will pass the tests which will compare your implementation
with Pytorch's implementation, but that's ok. So read the solution after 5 minutes of try & error
...
Why do we have to zero the gradient in PyTorch? PyTorch accumulates the gradient in the graph
Why do we use the word 'stochastic' in 'Stochastic gradient descent' in the context of deep learning? There are batches 
and the batch sizes are stochastics
...
Implement step:
    ...
    - weight_decay: What is the formula of the update when there is some weight_decay (ie when we penalize each 
        parameter squared)? Assume wd absorbs the constant.
...
There are multiple ways to implement SGD, so don't panic if there is some ambiguity and look at the solution to compare 
with the PyTorch implementation when you have used every variable.
"""
...
class _SGD:
    def __init__(
        ...
        self.params = list(params)  # model weights
        self.lr = lr  # learning rate
        self.wd = weight_decay  # decay rate, weight penalisation of the neural networks
        self.momentum = momentum  # here, it's the correct definition of momentum, kinetic of the movement
        self.dampening = dampening  # Tip: replace 1-momentum by 1-dampening, unused in PyTorch
...
class _Adam:
    def __init__(
        ...
        self.lr = lr
        self.beta1, self.beta2 = betas  # momentum of b1 and b2
        self.eps = eps
    ...
    def step(self):
        ...
        with torch.no_grad():
            for i, p in enumerate(self.params):
                ...
                # sum of gradient
                if self.b1[i] is not None:
                    self.b1[i] = self.beta1 * self.b1[i] + (1 - self.beta1) * g
                else:
                    self.b1[i] = g

                # sum of gradient squared
                if self.b2[i] is not None:
                    self.b2[i] = self.beta2 * self.b2[i] + (1 - self.beta2) * g**2
                else:
                    self.b2[i] = g**2
                ...
                # learning rate
                p -= self.lr * b1_hat / (b2_hat ** (0.5) + self.eps)
